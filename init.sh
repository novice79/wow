...
if [ ! -d "/var/lib/mysql/world" ]; then
    log "Copy wow server database ..."
    rm -rf /var/lib/mysql/*
    cp -r /tc-server/mysql/* /var/lib/mysql/
fi
...
if [ -n "$realmid" ]; then
    sed -i -E '/^RealmID/s/=.+$/= '"$realmid"'/' /tc-server/etc/worldserver.conf
fi
if [ -n "$logindb" ]; then
    sed -i -E '/^LoginDatabaseInfo/s/".+"/"'"$logindb"'"/' /tc-server/etc/worldserver.conf
fi
if [ -n "$name" ]; then
    mysql -u root -D auth -e "update realmlist set name='$name' where id=1;"
fi
if [ -n "$address" ]; then
    mysql -u root -D auth -e "update realmlist set address='$address' where id=1;"
fi
if [ -n "$port" ]; then
    mysql -u root -D auth -e "update realmlist set port='$port' where id=1;"
fi
cd /tc-server/bin
...
count=0
while [ 1 ]; do
    mysql -uroot -D auth -e "select id,username from account ;" | grep -i "$user"
    if [ $? -eq 0 ]; then
        screen -S ws -p 0 -X stuff "account set gmlevel $user 3 -1 \\r"
        #set this account to highest gm level 
        # mysql -uroot -D auth -e "select * from account_access;"
        # mysql -uroot -D auth -e "UPDATE account_access AS access
